...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    ...
    if (this.currentStep === 1) {
      if (!this.Kitchentype) {
        this.errors['Kitchentype'] = 'Please select a service.';
        valid = false;
      }
      ...
      if (!this.BriefRequirement.trim()) {
        this.errors['BriefRequirement'] = 'Please provide a brief description of your Kitchen remodeling needs.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        Kitchentype: parseInt(this.Kitchentype),
      ...
      this.http.post('https://get-kitchen.com/server/forward-lead', payload).subscribe({
        next: (response) => {
